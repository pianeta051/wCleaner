...
import { deleteFile, getFileUrl } from "../../services/files";
...
export const CustomerFiles: FC<CustomerFilesProps> = ({ customer, jobId }) => {
  ...
  const addFileHanlder = async (fileKeys: string[]) => {
    try {
  ...
  const deleteFileHandler = async (index: number) => {
    ...
    try {
      ...
      await replaceFiles(newFileKeys);
    } catch (error) {
      // the hook manages the error
    }
  };
  ...
  return (
    <>
      <Grid item xs={12} md={4}>
        <FileUploader customerId={customer.id} onSubmit={addFileHanlder} />
